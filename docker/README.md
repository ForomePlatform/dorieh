...
* For Amazon Linux (AMD64 only) - safe striped container without optional dependencies: [Dockerfile.amzn](Dockerfile.amzn)
...
DOCKER_BUILDKIT=1 BUILDKIT_PROGRESS=plain docker buildx build --platform linux/${platform} --no-cache --tag forome/dorieh:${platform}-${version} --load -f Dockerfile.${platform} . && docker push forome/dorieh:${platform}-${version}
```
    ...
    DOCKER_BUILDKIT=1 BUILDKIT_PROGRESS=plain docker buildx build --platform linux/arm64 --no-cache --tag forome/dorieh:arm64-${version} --load -f Dockerfile.arm64 . && docker push forome/dorieh:arm64-${version}
